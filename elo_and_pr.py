...
SPREADSHEET_ID = '1mOQm0ziJRMZ-JQPBnQmcFp6xHdyJphcGkUWqBiVeoAI'
...
# returns a list of match rows and columns for a user
def find_matches(match_data, username):
    match_string_list = []
    match_string_start = "Match History"
...
def initialize_parser():
    ...
    parser.add_argument("-d", "-deleteUser", nargs='?',
                        help="Deletes a user from the PR, as well as deleting all their matches played", type=str)

    # elo_and_pr {-r | -resetUser} [username]
    parser.add_argument("-r", "-resetUser", nargs='?',
                        help="Resets a user's ELO to 1500 as well as deleting all their matches played", type=str)
