...
def add_user(username):
    try:
        ...
        for user_data in elo_data:
            if username == user_data[0]:
                raise ValueError("This username exists in your PR already")
...
def add_match(winner, loser, gw, gl):
    try:
        ...
        winner_elo = get_sheet_data(winner_elo_str)
        loser_elo = get_sheet_data(loser_elo_str)
...
def list_top_x(top_num):
    try:
        ...
        top_users_list = [None] * top_num

        for user_info in elo_data:
            top_users_list[int(user_info[2]) - 1] = user_info[0]

        top_users_list = list(filter(None, top_users_list))
